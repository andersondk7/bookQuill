 ...
 * this is NOT a domain object but a raw database object.
 * The domain layer is responsible for:
 *   wire (json, etc.) translation
 *   data validation
 *   data types (rather than primitive string, int, float, etc.)
...
object BookTable {
  def toDomain(db: BookTable): Book = Book(
    id = ID.build(db.id),
  ...
  def fromDomain(book: Book): BookTable = BookTable(
